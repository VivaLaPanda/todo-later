...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Todo Later account.<br>
        ...
        &mdash; Todo Later Team
      `
      return sendMail({ toEmail: email, subject: 'Todo Later - Password Reset', content })
    })
